...
check(appJsText.includes("title: 'What we’re testing'"), 'pre-auth preview must present the two-idea creative test before account creation');
check(appJsText.includes("done: 'Screens ready to use'"), 'analysis route screenshot copy must stay customer-friendly');
check(appJsText.includes("done: 'Common themes found'"), 'analysis route review copy must describe the user outcome rather than the research pipeline');
check(appJsText.includes("'Planning your creative pack'"), 'paid generation must use customer-friendly planning copy');
check(appJsText.includes("'Creating image ads'"), 'paid generation must use customer-friendly image creation copy');
check(appJsText.includes("'Creating UGC videos'"), 'paid generation must use customer-friendly UGC creation copy');
check(appJsText.includes("'Preparing your files'"), 'paid generation must use customer-friendly delivery copy');
check(!appJsText.includes("'Matched product evidence'"), 'customer progress must not expose evidence-matching pipeline language');
check(!appJsText.includes("'Packaging exports'"), 'customer progress must not expose export-pipeline language');
...
check(appJsText.includes('deferPackPlan: true'), 'initial extraction must hand off directly into the same Creative Plan analysis route');
check(appJsText.includes("packPlanStatus: 'researching'"), 'the combined analysis route must continue into written-review research without exposing a second page');
check(appJsText.includes('Your creative plan is ready. Open “Why this plan?”'), 'combined preview completion must use one final reveal message');
check(!appJsText.includes('found. Building your creative plan now.'), 'first-run preview must not expose a secondary planning phase');
check(appJsText.includes("$('#importForm').hidden = showAnalysis"), 'the URL form must disappear throughout analysis and after extraction succeeds');
check(appJsText.includes('See how we built the creative plan for ${previewName}.'), 'the ready state must replace the paste-URL heading with plan-review context');
check(appJsText.includes('Try another app'), 'the ready state must keep a small, explicit way to restart with another app');
check(appJsText.includes("'Understanding your app…'"), 'anonymous store extraction must use a neutral loading label until the real app name is known');
