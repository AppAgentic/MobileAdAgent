  ...
  return {
    ...
    primary: {
      ...
      rationale: likedLearning
        ...
        : firstMarket
          ? 'This product message connects to a pattern in customer feedback.'
          : 'This leads with the clearest way your app helps.',
      evidenceRefs: unique([...primaryTruthRefs, primarySignalRef, tweakRef].filter(Boolean)),
    },
    challenger: {
      angle: challengerAngle,
      rationale: secondMarket
        ? 'This explores a different product message that also appears in customer feedback.'
...
function planAngleFromClaim(value) {
  ...
  const concise = words.join(' ').replace(/[,;:]+$/g, '');
