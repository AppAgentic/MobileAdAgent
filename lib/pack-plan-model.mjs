...
export function buildLearnedPackPlanStrategy({ reviewedApp = {}, researchSnapshot, publicResearch = {} } = {}) {
  ...
  if (!markets.length && !learnings.length) return null;
  ...
  const secondMarket = markets.find((item) => item.id !== firstMarket?.id) || null;
  const defaultProofAngle = 'Show the app in action';
  const defaultBenefitAngle = 'Lead with the main benefit';
  const marketPrimary = firstMarket ? strategyShortAngle(firstMarket.theme) : null;
  const primaryAngle = likedLearning?.angle
    || (marketPrimary && !rejectedAngles.has(copyKey(marketPrimary)) ? marketPrimary : null)
    || defaultProofAngle;
  const defaultAlternate = copyKey(primaryAngle) === copyKey(defaultProofAngle)
    ? defaultBenefitAngle
    : defaultProofAngle;
  let challengerAngle = secondMarket ? strategyShortAngle(secondMarket.theme) : defaultAlternate;
  if (copyKey(challengerAngle) === copyKey(primaryAngle) || rejectedAngles.has(copyKey(challengerAngle))) {
    challengerAngle = rejectedAngles.has(copyKey(defaultAlternate))
  ...
  return {
    ...
    primary: {
      ...
      rationale: likedLearning
        ...
        : firstMarket
          ? 'This matches the clearest pattern we found in public feedback.'
          : 'This keeps the clearest app evidence front and centre.',
      evidenceRefs: unique([...sharedRefs, primarySignalRef, tweakRef].filter(Boolean)),
    },
    challenger: {
      angle: challengerAngle,
      rationale: 'A different opening using the same app facts and screens.',
      evidenceRefs: unique([...sharedRefs, challengerSignalRef, tweakRef].filter(Boolean)),
    },
  };
}
...
export function customerSafeCreativePackPlan(plan) {
  ...
  const safe = {
    ...
    research: {
      ...
      productTruth: plan.researchSnapshot.productTruth.map((item) => ({
        id: item.id,
        kind: item.kind,
        text: item.text,
        source: compactObject({ label: item.source?.label, url: item.source?.url }),
...
function buildExploratoryStrategy(snapshot) {
  ...
  const commonRefs = unique([...claimIds.slice(0, 2), ...proofIds.slice(0, 2)]);
  const learningRefs = snapshot.learningSignals.slice(0, 2).map((signal) => signal.id);
  const primary = {
    id: 'primary',
    angle: 'Show the app in action',
    rationale: 'Start with a real app screen.',
    evidenceRefs: unique([...proofIds.slice(0, 2), ...claimIds.slice(0, 1), ...learningRefs]),
  };
  const challenger = {
    id: 'challenger',
    angle: 'Lead with the main benefit',
    rationale: 'Start with the clearest reviewed benefit, then show the app.',
    evidenceRefs: unique([...claimIds.slice(0, 2), ...proofIds.slice(0, 1), ...learningRefs]),
  };
  return {
    evidenceMode: 'exploratory',
    hypothesis: {
      statement: 'Showing the app first will be clearer than leading with a broad benefit.',
      audience: 'People whose needs match the reviewed app information.',
      tension: 'The clearest evidence is in the approved app screens.',
      valueConnection: 'Every ad stays grounded in the selected app facts and screens.',
      intendedLearning: 'Which opening should lead the next pack.',
      evidenceRefs: commonRefs,
    },
    experiment: experimentRecord({ primary, challenger, claimIds, proofIds }),
  };
}
...
function strategyShortAngle(value) {
  const cleaned = String(value || '').replace(/[.!?]+$/g, '').trim();
  if (!cleaned) return 'Show the app in action';
