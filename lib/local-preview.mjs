...
export function createPreviewStore({
  ...
  function cacheExtraction(canonicalAppId, extraction) {
    ...
    previewCache.set(canonicalAppId, { extraction, expiresAt: now() + cacheTtlMs, extractionCount });
  }
...
export function buildPreviewPayload(extraction, session) {
  const screenshots = (extraction.uiObjects || []).map((object, index) => ({
    ...
    url: object.sourceUrl || null,
    readiness: customerPreviewReadiness(object),
  }));
  const readyCount = screenshots.filter((shot) => shot.readiness.status === 'recommended').length;
  const reviewCount = screenshots.filter((shot) => shot.readiness.status === 'review').length;
  const blockedCount = screenshots.filter((shot) => shot.readiness.status === 'blocked').length;
  ...
  return {
    ...
    readiness: {
      ...
      reviewCount,
      blockedCount,
      gaps: previewGaps(extraction, screenshots),
    },
    access: {
...
function customerPreviewReadiness(object = {}) {
  ...
  const internalCopy = /rawif|pre_rawification|ui_extracted|store_art|raw_app_proof/i.test(
    `${object.usability?.label || ''} ${object.usability?.reason || ''}`,
  );
  if (internalCopy) {
    return {
      status,
  ...
  return {
    status,
    label: object.usability?.label || (status === 'recommended' ? 'Ready for ads' : status === 'blocked' ? 'Not used' : 'Needs review'),
    reason: object.usability?.reason || 'Review this screenshot after you claim the preview.',
  };
}

function previewGaps(extraction, screenshots) {
  const gaps = [];
  if (!screenshots.length) {
    gaps.push('No screenshots were found on the listing. You can add your own after you claim the preview.');
  } else if (!screenshots.some((shot) => shot.readiness.status === 'recommended')) {
    gaps.push('Screenshots were found, but none look ad-ready yet. Review them after you claim the preview.');
  }
  if (!(extraction.claimCandidates || []).length) {
    gaps.push('No clear key features were found. Add one true product feature after you claim the preview.');
  }
  if (!extraction.app?.summary) {
    gaps.push('No app summary was found. You can write one after you claim the preview.');
  }
  return gaps;
}
