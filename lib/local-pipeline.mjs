...
const DEFAULT_CONSTRAINTS = {
  ...
  fps: 30,
  renderBackend: 'local-hyperframes-preview',
  generationBackend: 'mock-kling-seedance-local',
  providerMutations: 0,
};
...
function buildCreativeBrief(appProfile, proofs, constraints) {
  ...
  return {
    ...
    acceptanceCriteria: [
      ...
      'Moves proof cutaways through HyperFrames timeline assembly.',
...
function buildStages({
  ...
  const stageData = {
    ...
    proof_prep: {
      ...
      output: `${proofPrep.assets.length} proof beats cropped/timestamped for HyperFrames.`,
    },
    hyperframes_render: {
  ...
  return {
    ...
    lineItems: [
      ...
      },
      {
        ...
        estimatedCost: 0,
        note: 'Mocked locally; no HeyGen call.',
      },
      {
        stage: 'qa_agent',
        backend: 'local-rules',
        estimatedCost: 0,
        note: 'No Gemini call in local prototype.',
      },
    ],
  };
}
